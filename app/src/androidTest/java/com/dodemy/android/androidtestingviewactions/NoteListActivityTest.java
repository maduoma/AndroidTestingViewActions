...
public class NoteListActivityTest {
    ...
    public void noteListActivityTest() {
        ...
        ViewInteraction appCompatEditText = onView(allOf(withId(R.id.text_note_title), isDisplayed()));
        appCompatEditText.perform(replaceText("Title"), closeSoftKeyboard());

        ViewInteraction appCompatEditText2 = onView(allOf(withId(R.id.text_note_text), isDisplayed()));
        appCompatEditText2.perform(replaceText("Body"), closeSoftKeyboard());

        ViewInteraction editText = onView(allOf(withId(R.id.text_note_title), withText("Title"),
                        childAtPosition(childAtPosition(IsInstanceOf.<View>instanceOf(android.view.ViewGroup.class),
                                1), 1), isDisplayed()));editText.check(matches(withText("Title")));
        pressBack();
        //pressBack();
    }
